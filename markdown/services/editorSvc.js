...
const editorSvc = Object.assign(createApp(document.createElement('div')), editorSvcDiscussions, editorSvcUtils, {
  // Elements
  editorElt: null,
  previewElt: null,
  tocElt: null,
  ...
  convert() {
    ...
    emitter.emit('conversionCtx', this.conversionCtx);
  ...
  refreshPreview() {
    ...
    emitter.emit('previewHtml', this.previewHtml)
    ...
    emitter.emit('previewText', this.previewText);
    ...
    const onEditorChanged =
      (sectionList = this.sectionList, selectionRange = this.selectionRange) => {
        if (this.sectionList !== sectionList) {
          ...
          emitter.emit('sectionList', this.sectionList)
          if (instantPreview) {
            refreshPreview();
          } else {
        ...
        if (this.selectionRange !== selectionRange) {
          ...
          emitter.emit('selectionRange', this.selectionRange);
        }
